...
function Write-LogFileSafe {
    ...
    for ($attempt = 1; $attempt -le $MaxRetries; $attempt++) {
        try {
            # AppendAllText is typically more resilient than Add-Content under lock contention
            ...
            # Logging must never crash the toolkit
...
function Write-Log {
    ...
    # File output (toolkit.log). Important: this must NOT be transcript.log.
    if (-not $NoFile) {
        try {
